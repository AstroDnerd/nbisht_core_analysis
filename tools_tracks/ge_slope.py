...
#            pch.helper(hist,xb,yb,ax=ax0,transpose=False)
        if 1:
            #get and plot the upper envelope
            #Clean out the scatter of single points
            #by comparing to neighbors.
            h2 = hist+0
            ...
            ok = np.logical_and(DUE > 0, r_cen[keepers][1:]>1e-3)
            ok = np.logical_and(ok, UE[1:]<0.5*cummax[1:])

        if ok.any():
            index = np.where(ok)[0][0]
            R_KEEP = r_cen[keepers][index]
            #ax.plot(r_cen[keepers], cummax,c='g')
            ax.scatter( r_cen[keepers][index],UE[index],c='r')
            #fit until the bump.
            ok = np.logical_and(r_sphere < R_KEEP, GE>0)
            ok = np.logical_and(ok, r_sphere > r_cen[keepers].min())

            rok=r_sphere[ok]
            def powerlaw( x, q, r0):
                return q*x+r0
            popt, pcov=curve_fit(powerlaw, np.log10(rok), np.log10(GE[ok]))
