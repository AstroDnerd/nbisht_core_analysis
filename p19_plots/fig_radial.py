...
def replotter(obj,suffix1='', redlines=False, subset=0):
    ...
    if 'vr_cumsum' in profs:
        ...
        for nax,ax in enumerate(axes[row]):
            vr_ylim=ext[row].minmax
            vr_ylim = -4,2
            ax.set(xscale='log',yscale='linear',ylim=vr_ylim, ylabel=r'$\overline{v_{\rm{R}}(<r)}/c_s$', xlim=ext[-1].minmax)
    ...
    if 'vt_cumsum' in profs:
        ...
        for nframe,ax in enumerate(axes[row]):
            ax.set(xscale='log',yscale='linear',ylim=ext[row].minmax, ylabel=r'$\overline{v_{\rm{T}}(<r)}/c_s$', xlim=ext[-1].minmax)
