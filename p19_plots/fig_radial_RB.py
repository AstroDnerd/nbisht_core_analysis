    ...
    def run(self, core_list=None, frame_list=None, tsing=None):
        ...
        if Nplots > 4:
            ...
            for core_id in core_list:
                ...
                rho_ext(ms.density)
        r = np.geomspace(1e-3,1e-1)
        axes[0][3].plot(r,r**-2, c=[0.5]*4,linestyle='--')
        for nframe in range(Ntimes):
            axes[1][nframe].axhline(1,c=[0.5]*4)
            axes[3][nframe].axhline(2,c=[0.5]*4)
            axes[3][nframe].axhline(0.5,c=[0.5]*4)
        for core_id in core_list:
            ms = trackage.mini_scrubber(this_looper.tr,core_id)
            ...
            frame_mask[0]=True
            fraction_of_tsing=0.6
            frame_mask[get_time_index(fraction_of_tsing*tsing.tsing_core[core_id])]=True
            ...
            axes[0][1].set_title(r'$t=%0.1f t_{\rm{sing}}$'%fraction_of_tsing)
            ...
            for nframe,frame in enumerate(frame_list):
                ...
                bx = sph['magnetic_field_x']
                by = sph['magnetic_field_y']
                bz = sph['magnetic_field_z']
                bP = (bx*bx+by*by+bz*bz)/(8*np.pi)
                ...
                if 0:
                    ...
                    ext[1](vr_cumsum)
                if 1:
                    bP_cumsum=np.cumsum( rho_sort/bP_sort*dv_sort)/V_cuml
                    axes[1][nframe].plot(RR_sort,bP_cumsum,**args)
                    #axes[1][nframe].axhline(1,c=[0.5]*4)
                    ext[1](bP_cumsum)
        ...
        if 0:
            ...
            for ax in axes[1]:
                ax.set(xscale='log',yscale='log',ylim=ext[1].minmax, ylabel=r'$\beta$', xlim=ext[-1].minmax)
        for ax in axes[2]:
        ...
        for ax in axes[-1]:
            ax.set(xlabel='r')
        fig.savefig('plots_to_sort/radial_profile_play_%s'%(this_looper.sim_name))
