...
def magnetic_angle(field,data):
    vx = data['velocity_x'].v
    vy = data['velocity_y'].v
    vz = data['velocity_z'].v
    bx = data['magnetic_field_x'].v
    by = data['magnetic_field_y'].v
    bz = data['magnetic_field_z'].v
    ...
    out = np.zeros_like(data['velocity_x']/data['velocity_x'])
    out[:] = np.arccos((vx*bx+vy*by+vz*bz)/(vv*bb))*180/np.pi
    return out
...
def test_angle():
    x,y,z = np.mgrid[0:1:0.1,0:1:0.2,0:1:0.1]
    test_data = {}
    test_data['velocity_x']       = np.zeros_like(x) + 1
    test_data['velocity_y']       = np.zeros_like(x)
    test_data['velocity_z']       = np.zeros_like(x)
    test_data['magnetic_field_x'] = np.zeros_like(x)  + 1
    test_data['magnetic_field_y'] = np.zeros_like(x) 
    test_data['magnetic_field_z'] = np.zeros_like(x)

    out = magnetic_angle(None, test_data)
    return out
#angle_test = test_angle()
...
##                           #zoom=False, grids=False, particles=False, moving_center=True) #, frame_list=[1])
loop_apps.core_proj_multiple(this_looper,axis_list=[0], field=FIELD, color_dict=color_dict, particles=False, fields=True, velocity=False, frame_list=[0] )#, frame_list=[0], moving_center=False)
