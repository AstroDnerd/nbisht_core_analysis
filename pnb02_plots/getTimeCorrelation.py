...
path_to_output_plots = "/home/x-nbisht1/projects/results/512/NonsinkSimSuite/TimeCorrelation/"
...
def plot_autocorrelation_0d(x):
    ...
    plt.tight_layout()
    plt.savefig(path_to_output_plots+"/AutocorrelationTest_0D.png")
...
def plot_autocorrelation_nd(x, dim='1D'):
    ...
    for i in range(len(x)):
        ...
        plt.savefig(path_to_output_plots+"/AT_"+dim+"_plots/DD%04d.png"%(i))
        plt.close()
    animator(path_to_output_plots+"/AT_"+dim+"_plots","AutocorrelationTest_"+dim)
    
def paperplot_autocorrelation(x, simname='nb101'):
    print(simname)
    #calculate Nd autocorrelation
    ac_x = ac_fft(x)
    print("FFT shape:",ac_x.shape)
    ...
    plt.savefig(path_to_output_plots+"/Densityvar_autocorrelation_"+simname+".png")
    plt.close()
...
if 1:
    ...
    for sim in simarray[0:2]:
        for dd in ddnumber[0:2]:
            ...
            paperplot_autocorrelation(TSCube_density, simname = datasetname)
