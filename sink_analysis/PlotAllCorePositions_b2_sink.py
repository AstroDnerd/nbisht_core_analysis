...
#get core positions
f = h5py.File("/data/cb1/nbisht/anvil_scratch/projects/128/B2_sink_nazare/datasets/nb102_0088_peaklist.h5","r")
...
#plot peak locations
peak_densities = []
peak_densities_above1000 = []
for peak_index in range(num):
    ...
    if float(peak_density)>1000:
        peak_densities_above1000.append(float(peak_density))
...
print(min(peak_densities_above1000),max(peak_densities_above1000), sum(peak_densities_above1000)/len(peak_densities_above1000),len(peak_densities_above1000))
...
fig = plt.figure()
plt.hist(peak_densities_above1000)
...
import json
with open('/data/cb1/nbisht/anvil_scratch/projects/128/B2_sink_nazare/datasets/sink_clump_position_with_sink_totaldensity.json', 'w') as fp:
