...
def why(this_looper,core_list=None, do_plots=True, r_inflection=None, frame_list=None, tsing=None):
    ...
    for core_id in core_list:
        ...
        R_G_K = np.zeros( [len(r_bins)-1, len(frame_list)])
        R_T_K = np.zeros( [len(r_bins)-1, len(frame_list)])
        R_B_K = np.zeros( [len(r_bins)-1, len(frame_list)])
        for nframe,frame in enumerate(frame_list):
            ...
            ET = np.abs(sp[YT_therm_energy])
            EB = np.abs(sp[YT_magnetic_field_strength])/(8*np.pi)
            ...
            ET_cuml = np.cumsum( ET[ORDER]*V_local)/V_cuml
            EB_cuml = np.cumsum( EB[ORDER]*V_local)/V_cuml
            ...
            ETmeans =nar([ ET_cuml[ digitized == i].mean() if (digitized==i).any() else -4000 for i in range(1,len(r_bins))])
            EBmeans =nar([ EB_cuml[ digitized == i].mean() if (digitized==i).any() else -4000 for i in range(1,len(r_bins))])

            
            ok = EKmeans>0
            R_G_K[ok,nframe]=(EGmeans/EKmeans)[ok]
            R_T_K[ok,nframe]=(ETmeans/EKmeans)[ok]
            R_B_K[ok,nframe]=(EBmeans/EKmeans)[ok]
        ...
        for ne,RV in enumerate([R_G_K, R_T_K, R_B_K]):
            ax=axes[0,ne]
            ax1=axes[1,ne]
        ...
        fig.savefig('plots_to_sort/energies_%s_c%04d.png'%(this_looper.sim_name, core_id))
...
if 1:
    for sim in sim_list:
        ...
        frame_list = list(all_frames[::10]) + list(all_frames[-1:])
        #frame_list = [100]
