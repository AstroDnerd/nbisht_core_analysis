...
class multi_profile():
    ...
    def run(self, core_list=None, frame_list=None, tsing=None):
        ...
        for ax in axes:
            ax.set(xscale='log',yscale='log',xlabel=r'$\rho$',ylabel='', xlim=ext_r.minmax)#, ylim=ext.minmax)
        axes[0].set(ylabel=r'$PDF(\rho)$')
        for ax in axes[1:]:
            ax.set(yticks=[])
        fig.savefig('plots_to_sort/fig_angular_momentum_profile_%s'%(this_looper.sim_name))
