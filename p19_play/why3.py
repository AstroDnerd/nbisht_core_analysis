...
def why(this_looper,core_list=None, do_plots=True, r_inflection=None, frame_list=None, tsing=None):
    ...
    for core_id in core_list:
        ...
        for nframe,frame in enumerate(frame_list):
            ...
            this_time = this_looper.tr.times[nf]/colors.tff
            ...
            hist, xbins,ybins=np.histogram2d(RR.flatten(),EG.flatten(),bins=[r_bins,yybins])
            pch.helper(hist,xbins,ybins,ax=axes[0][0])
            axes[0][0].scatter( msR[:,nf], np.abs(ms.ge[:,nf]), facecolor='None', edgecolor='r',s=30, alpha=0.4)
            axbonk(axes[0][0],xscale='log',yscale='log',xlabel='R',ylabel='GE')
            axes[0][0].set_title('t=%0.4f'%this_time)

            hist, xbins,ybins=np.histogram2d(RR.flatten(),EK.flatten(),bins=[r_bins,yybins])
            pch.helper(hist,xbins,ybins,ax=axes[1][0])
            axes[1][0].scatter( msR[:,nf], np.abs(ms.ke[:,nf]), facecolor='None', edgecolor='r',s=30, alpha=0.4)
            axbonk(axes[1][0],xscale='log',yscale='log',xlabel='R',ylabel='KE')

            dbins = np.geomspace( 1e-2,1e7, 64)
            hist, xbins,ybins=np.histogram2d(RR.flatten(),DD.flatten(),bins=[r_bins,dbins])
            pch.helper(hist,xbins,ybins,ax=axes[1][1])
            axes[1][1].scatter( msR[:,nf], ms.density[:,nf], facecolor='None', edgecolor='r',s=30, alpha=0.4)
            axbonk(axes[1][1],xscale='log',yscale='log',xlabel='R',ylabel='density')


            proj=ds.proj('density',0,data_source=sp)
            pw=proj.to_pw()
            pw.zoom( 1/Radius)
            xax = ds.coordinates.x_axis[0]
            yax = ds.coordinates.y_axis[0]
            pw.set_center( [c[xax],c[yax]])
            pw._setup_plots()

            norm = mpl.colors.LogNorm( vmin=1e-2, vmax=1e4)
            axes[0][1].imshow( pw.frb['density'], norm=norm)
            

            fig.savefig('plots_to_sort/why45_%s_c%04d_n%04d.png'%(this_looper.sim_name,core_id,frame))
        ...
        fig.savefig('plots_to_sort/why2b_%s_c%04d.png'%(this_looper.sim_name, core_id))
