...
def hist_loss(pred, true, bins=16):
    #CURRENTLY FLATTENS IT ALL TO 1D, MAY CHANGE TO IMPLEMENT it to B,N,1?
    # flatten
    p = pred.view(-1)
    t =  true.view(-1)
...
class UNet2D(nn.Module):
    def __init__(self, in_channels, out_channels, base_filters=32):
        ...
        self.enc1 = nn.Sequential(
            nn.Conv2d(in_channels, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_filters, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
        )
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.enc2 = nn.Sequential(
            nn.Conv2d(base_filters, base_filters*2, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_filters*2, base_filters*2, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
        )
        # (You can add more encoder levels here…)

        # Attention Gate
        # For skip from enc1 to dec2:
        # F_g = base_filters (from decoder),
        # F_l = base_filters (from encoder),
        # F_int = base_filters // 2
        self.att2 = AttentionGate(F_g=base_filters, F_l=base_filters, F_int=base_filters // 2)
        
        # Decoder path
        self.upconv2 = nn.ConvTranspose2d(base_filters*2, base_filters, kernel_size=2, stride=2)
        self.dec2 = nn.Sequential(
            nn.Conv2d(base_filters*2, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_filters, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
        )
        ...
        self.outconv = nn.Conv2d(base_filters, out_channels, kernel_size=1, padding_mode='circular')
    ...
    def forward(self, x):
        ...
        # Attention gating on e1 using decoder signal d2
        e1_att = self.att2(e1, d2)                  # [B, F, H, W]

        d2 = torch.cat([d2, e1_att], dim=1)        # skip connection [B, 2F, H, W]
...
def train_unet(input_arr, output_arr, labels, args):
    ...
    if args.loss_type == 'static':
        ...
        with torch.no_grad():
            ...
            # Histogram loss
            hist_0 = hist_loss(p0, y0, bins=32)
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            #Histogram Loss
            hist_l = hist_loss(pred, y, bins=32)
    ...
    # === Save & plot ===
    torch.save(model.state_dict(), args.run_name+'_'+args.loss_type+'_'+MODELFILE)
    if args.loss_type != 'static':
...
def plot_comparison(input_arr, pred_output_arr, act_output_arr, label, save_filename='xy_data.png', dont_show = False):
    ...
    input_FFTL = F.mse_loss(torch.log1p(torch.abs(torch.fft.fft2(torch.from_numpy(input_arr)))), torch.log1p(torch.abs(torch.fft.fft2(torch.from_numpy(act_output_arr)))))
    input_str+= 'FFTL: '+'{:0.5f}'.format(input_FFTL)+'\n'
    pred_FFTL = F.mse_loss(torch.log1p(torch.abs(torch.fft.fft2(torch.from_numpy(pred_output_arr)))), torch.log1p(torch.abs(torch.fft.fft2(torch.from_numpy(act_output_arr)))))
    pred_str+= 'FFTL: '+'{:0.5f}'.format(pred_FFTL)+'\n'
    actual_FFTL = F.mse_loss(torch.log1p(torch.abs(torch.fft.fft2(torch.from_numpy(act_output_arr)))), torch.log1p(torch.abs(torch.fft.fft2(torch.from_numpy(act_output_arr)))))
    act_str+= 'FFTL: '+'{:0.5f}'.format(actual_FFTL)+'\n'

    fig = plt.figure(figsize=(12, 4))
    
    ax  = fig.add_subplot(1, 3, 1)
    c = ax.pcolormesh(input_arr.T, vmin=-2, vmax=2, cmap='gnuplot2')
    ...
    c = ax2.pcolormesh(pred_output_arr.T, vmin=-2, vmax=2, cmap='gnuplot2')
    ...
    c = ax3.pcolormesh(act_output_arr.T, vmin=-2, vmax=2, cmap='gnuplot2')
