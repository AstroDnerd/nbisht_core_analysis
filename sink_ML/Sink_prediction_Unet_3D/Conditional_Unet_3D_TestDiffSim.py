...
MODELFILE = 'Unet_3D_dynamic_DeeperAttention_4Loss_nnmodel.dict'
...
def get_images(input_dir):
    output_dic = {}
    for filename in os.listdir(input_dir)[::100]:  # Load every 100 file to reduce memory usage
...
def compare_output(input_arr, output_arr, labels):
    ...
    with torch.no_grad():
        for i in range(len(input_arr)):
            ...
            plot_comparison(inputs[0][0].numpy(), output_image[0][0], targets[0][0].numpy(), label, Loss_dict, save_filename='./3D_u502_pred/img'+"_".join(str(x) for x in label)+'.png', dont_show = True)
    ...
    plt.savefig('Unet3D_Deeper_u502Test.png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()


compare_output(input_image_arr, output_image_arr, label_arr)