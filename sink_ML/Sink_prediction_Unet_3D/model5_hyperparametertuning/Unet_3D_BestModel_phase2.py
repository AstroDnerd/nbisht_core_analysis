...
#There are over 10000 models in the phase2_model_performance.csv file, so we will prune them based on their performance
#We will keep the best models based on avg_test_MSE
#We will prune the models every 2 epochs, starting from epoch 2, until epoch 20
#The pruning levels are as follows

pruning_epochs = np.array([4, 8, 15])
pruning_number = np.array([0.5, 0.5, 10])  # 50% for first 2 levels, then fixed number of models to keep
from filelock import FileLock

#if a csv file exists, load it and return that as df
lock = FileLock("phase2_model_performance.csv.lock")
pruning_level = 0
with lock:
    if os.path.exists("./phase2_model_performance.csv"):
        all_model_df = pd.read_csv("./phase2_model_performance.csv")
    else:
        all_model_df = all_models_phase_two()
    ...
    if pruning_level==2 and len(model_df) > 14:
        #randomly choose 14 models from the dataframe
        model_df = model_df.sample(n=14, random_state=seed)
        print(f"Randomly selected {len(model_df)} models for training at pruning level {pruning_level}", flush=True)
    #make the training column 1 for these models in original dataframe
    all_model_df.loc[model_df.index, 'isTraining'] = 1
    #save the updated dataframe
    all_model_df.to_csv("./phase2_model_performance.csv", index=False)
...
with lock:
    all_model_df = pd.read_csv("./phase2_model_performance.csv")  #reload dataframe incase it changed during training
    ...
    if len(all_model_df['current_epoch'].unique()) == 1:
        ...
        if pruning_level < len(pruning_epochs):
            ...
            all_model_df.to_csv(f"./pruned_models/phase2_model_performance_{pruning_level}.csv", index=False)
            #select the models that are above the threshold
            all_model_df = all_model_df[all_model_df['keep_model'] == 1]

    print("Saving model performance results to phase2_model_performance.csv", flush=True)
    all_model_df.to_csv(f"./phase2_model_performance.csv", index=False)
