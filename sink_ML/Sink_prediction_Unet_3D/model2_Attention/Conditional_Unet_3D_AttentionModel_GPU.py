...
class UNet3D(nn.Module):
    def __init__(self, in_channels, out_channels, base_filters=32):
        ...
        self.enc1 = nn.Sequential(
            nn.Conv3d(in_channels, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
            nn.Conv3d(base_filters, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
        )
        self.pool = nn.MaxPool3d(kernel_size=2, stride=2)
        self.enc2 = nn.Sequential(
            nn.Conv3d(base_filters, base_filters*2, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
            nn.Conv3d(base_filters*2, base_filters*2, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
        )
        ...
        # Decoder path
        self.upconv2 = nn.ConvTranspose3d(base_filters*2, base_filters, kernel_size=2, stride=2)
        self.dec2 = nn.Sequential(
            nn.Conv3d(base_filters*2, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
            nn.Conv3d(base_filters, base_filters, kernel_size=3, padding=1, padding_mode='circular'),
            nn.ReLU(inplace=True),
        )
        ...
        self.outconv = nn.Conv3d(base_filters, out_channels, kernel_size=1, padding_mode='circular')

    def forward(self, x):
        # Encoder
        e1 = self.enc1(x)                          # [B, F, D, H, W]
        e2 = self.enc2(self.pool(e1))              # [B, 2F, D/2, H/2, W/2]
        # (encode deeper levels…)

        # Decoder
        d2 = self.upconv2(e2)                      # [B, F, D/2, H/2, W/2]
        # Attention gating on e1 using decoder signal d2
        e1_att = self.att2(e1, d2)                  # [B, F, D, H, W]
        d2 = torch.cat([d2, e1_att], dim=1)        # skip connection  [B, 2F, D, H, W]
        d2 = self.dec2(d2)                         # [B, F, D, H, W]
        # (decode through further levels…)

        return self.outconv(d2)                    # [B, out_channels, D, H, W]
...
def train_unet(input_arr, output_arr, labels, args):
    ...
    # === Save & plot ===
    torch.save(model.state_dict(), args.run_name+'_'+args.loss_type+'_Attention_'+MODELFILE)
    if args.loss_type != 'static':
        torch.save(loss_w.detach().cpu(), "./plots/loss_weights_attention.pt")
    import matplotlib.pyplot as plt
    print(loss_history)
    ...
    plt.savefig(f"./plots/{args.run_name}_attention_training_loss.png")
    plt.close()
...
def compare_output(input_arr, output_arr, labels, args):
    ...
    model = UNet3D(in_channels=1, out_channels=1).to(DEVICE)
    model.load_state_dict(torch.load(args.run_name+'_'+args.loss_type+'_Attention_'+MODELFILE))
    model.eval()
