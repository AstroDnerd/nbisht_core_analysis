...
def compare_output(input_arr, output_arr, labels, args):
    ...
    with torch.no_grad():
        for i in range(len(input_arr)):
            ...
            plot_comparison(inputs[0][0].numpy(), output_image[0][0], targets[0][0].numpy(), label, save_filename='./3D_model_perf/img'+"_".join(str(x) for x in label)+'.png', dont_show = True)
