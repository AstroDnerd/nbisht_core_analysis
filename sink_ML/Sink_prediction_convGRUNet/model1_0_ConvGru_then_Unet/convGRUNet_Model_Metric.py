...
def plot_comparison(input_arr, pred_output_arr, act_output_arr, label, title, save_filename='xy_data.png', dont_show = False):
    # Calculate metrics
    ...
    ax.set_title(title+'\n Input: '+str(label[0])+'_'+str(label[1])+'_'+str(label[2]))
    ax.grid('both')
...
def compare_output(input_arr, output_arr, labels, args, dir_name ='3D_model_perf'):
    ...
    with torch.no_grad():
        for i in range(len(input_arr)):
            ...
            plot_comparison(inputs[0][0].numpy(), output_image[0][0], targets[0][0].numpy(), label, title = args.run_name, save_filename='./'+dir_name+'/img'+"_".join(str(x) for x in label)+'.png', dont_show = True)
...
compare_output(input_image_test, output_image_test, label_test, args, dir_name ='3D_model_perf_scaled')