...
def train_unet_unified_delta_chunked_DDP(world_size, THREADS_PER_PROC, modelclass, chunk_manager, args, argsGRU, validate_dataset=True, dont_save=False):
    make_dir(f"models/plots/")
    os.environ["OMP_NUM_THREADS"] = str(THREADS_PER_PROC)
    os.environ["MKL_NUM_THREADS"] = str(THREADS_PER_PROC)

    print(f"Spawning {world_size} processes...", flush=True)
    mp.spawn(
        train_unet_unified_delta_chunked_DDP_trainer,
        args=(world_size, THREADS_PER_PROC, modelclass, chunk_manager, args, argsGRU, validate_dataset, dont_save),
        nprocs=world_size,
        join=True
    )

def train_unet_unified_delta_chunked_DDP_trainer(rank, world_size, THREADS_PER_PROC, modelclass, chunk_manager, args, argsGRU, validate_dataset=True, dont_save=False):
    """
    Optimized training function with delta training with mixed precision and improved memory management and gradient accumulation.
    Notes:
    - HDF5 datasets
    ...
    # Pin process to specific CPU cores to avoid NUMA issues
    # rank 0: 0-31, rank 1: 32-63, etc.
    os.sched_setaffinity(0, range(rank * THREADS_PER_PROC, (rank + 1) * THREADS_PER_PROC))
    
    print(f"Rank {rank}: Started on cores {rank*THREADS_PER_PROC}-{(rank+1)*THREADS_PER_PROC}", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, chunk_keys in enumerate(chunk_manager.train_chunks):
            ...
            # DDP Sampler ensures each rank gets DIFFERENT data from the chunk
