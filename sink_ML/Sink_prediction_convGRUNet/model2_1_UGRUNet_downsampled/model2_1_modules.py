...
def train_unet_delta(input_arr, output_arr, labels, args, argsGRU):
    X = torch.stack(input_arr, dim=0).float().unsqueeze(2)  # [N,t, 1,16, 16, 16]
    Y = torch.stack(output_arr, dim=0).float().unsqueeze(1) # [N, 1,16, 16, 16]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch in epoch_ranges:
            ...
            for g in optimizer.param_groups:
                g['lr'] = args.lr
            torch.save({'epoch': args.epochs, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 
                'scheduler_state_dict': LRSched.state_dict(), 'loss': loss_history, 'val_loss': val_loss_history}, f"models/{args.run_name}_chk{str(epoch)}_{MODELFILE}")
            torch.save(loss_w.detach().cpu(), f"models/plots/loss_data_{args.run_name}_chk{str(epoch)}_loss.pt")
