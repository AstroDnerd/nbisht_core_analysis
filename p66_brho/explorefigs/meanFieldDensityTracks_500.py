    ...
    def profiles(self,name=None):
            ...
            for nf,theframe in enumerate(thtr.frames): 
                ...
                if 1:  #ALL CORE DATA 
                    ...
                    # fields:
                    Cellvolume_all_cores_all_time = thtr.track_dict['cell_volume']  # shape: (particles,14) for u501
                    CellVolume = Cellvolume_all_cores_all_time[:,nf]  # shape: (particles,) for u501 
...
if 'mag_den1' not in dir() or clobber:
    mag_den1=magfield_density_tool(TL500.loops['u501'])
    simname1 = 'u501'
    mag_den1.run()
    #mag_den1.profiles(simname1)

if 'mag_den2' not in dir() or clobber:
    mag_den2=magfield_density_tool(TL500.loops['u502'])
    simname2 = 'u502'
    mag_den2.run()
    #mag_den2.profiles(simname2)

if 'mag_den3' not in dir() or clobber:
    mag_den3=magfield_density_tool(TL500.loops['u503'])
    simname3 = 'u503'
    mag_den3.run()
