    ...
    if 1: 
        ...
        for i in range(3):
            ...
            if 0: 
                ...
                pdf, bins = np.histogram(np.delete(diff_theta[i],index), bins=the_bins, density=False, weights=the_weights)
                cdf= np.cumsum(pdf)
                bin_centers = 0.5*(bins[1:]+bins[:-1]) 
                plt.plot(bin_centers,cdf,c=color[i],alpha=0.7)
                ...
                plt.ylabel('CDF')
                outname ='theta_diff_CDF_%d_%s'%(i,simnames[nt])
                #outname ='theta_diff_CDF_%s'%(simnames[nt])
            if 1: 
                the_bins = 16 
                the_weights = np.ones(len(np.delete(relerr_UQfrac[i],index)))/len(np.delete(relerr_UQfrac[i],index))
                pdf, bins = np.histogram(np.delete(relerr_UQfrac[i],index), bins=the_bins, density=False, weights =the_weights )
                cdf= np.cumsum(pdf)
                bin_centers = 0.5*(bins[1:]+bins[:-1]) 
                plt.plot(bin_centers,cdf,c=color[i],alpha=0.7)
                plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                ...
                plt.ylabel('CDF')
                outname = 'UQfrac_relerr_CDF_%d_%s'%(i,simnames[nt])
                #outname = 'UQfrac_relerr_CDF_%s'%(simnames[nt])
