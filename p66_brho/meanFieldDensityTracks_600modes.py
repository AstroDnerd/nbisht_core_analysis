...
if 0:
    ...
    for nt,tool in enumerate([mag_den1,mag_den2,mag_den3]):   
        # SET UP THE VARIABLE
        atf[nt] = []  #keep track of the atf alphas
        ...
        if 1:
            ...
            for i in range(coreint): 
                ...
                if 1:
                    ...
                    outname_frame='Scatter_LogBvsRho_mFilament_%s_%d'%(simnames[nt],i)
                    #outname_frame='Scatter_LogBzvsRho_%s_%d%(simnames[nt],i)
                    figs[i].savefig(outname_frame)
                    print("saved ",i)
                    plt.close(figs[i])
        ...
        # record respective atf alphas in RECORDS
        print('atf_list',atf[nt])
        #alphaFile = open("p66_brho/alphaRecords.txt",'a')
        #alphaFile.write("Sim %d alpha_mOne %f \n"%(nt,atf[nt]))
        #alphaFile.close()
    if 0:
...
if 1:  #r: one, g: binary, b: cluster, k: filament
    fig, ax1=plt.subplots(1,1) 
    therange = np.arange(0,1,0.075)
    ax1.plot(therange, [0.18941526470836778, 0.20375557122369464, 0.23159105289239096,
        0.22841537157091604, 0.1705604025733461, 0.11313124268767354,
        0.15817468925287423, 0.11399800833871786, 0.06884968886672427,
        0.13383907863131353, 0.14771282880215358, 0.14149811236996904,
        0.26108023592111657, 0.44640415774219294], c='r')
    ax1.plot(therange, [0.23761890305830308, 0.23525008642160647, 0.29660793647359557,
        0.30635882962270305, 0.20147263853705055, 0.06520599759539887,
        0.08517897982855652, 0.06582393783346145, 0.03437668398594257,
        0.05545442761551973, 0.1476999745675448, 0.2616008145467559,
        0.38496235247735905, 0.482137252199542], c='g')
    ax1.plot(therange, [0.3121141852454351, 0.31217081370881006, 0.1779425006904712,
        0.13331791420110353, 0.08518589070352715, 0.039277581704443204,
        0.09810047489281892, 0.12193436903837093, 0.014246100548649752,
        -0.018279174383280642, 0.024924101880451546, 0.10392286406057288,
        0.37706369028145514, 0.3841419054141555], c='b')
    ax1.plot(therange, [-0.01142422845017509, -0.1020301160081093, -0.018359500547339758,
        0.0809619568172576, 0.1688252041988971, 0.1354893434902574, 0.12182160006829719,
        0.10965466575989612, 0.03778503565859694, 0.08052256475327552,
        0.14765302560773078, 0.33953683646771016, 0.5742545321814714,
        0.41298075873381035], c='k')

    ax1.set_xlabel(r'$t/t_{\rm{ff}}$')
    ax1.set_ylabel(r'$\alpha_O,\alpha_B,\alpha_C,\alpha_F$')
    fig.savefig('simOne')
    print(" saved")
    plt.close(fig)
    ...
    fig, ax1=plt.subplots(1,1) 
    therange = np.arange(0,1,0.075)
    ax1.plot(therange, [0.3187846697473608, 0.2604491698238738, 0.2682979508519205,
        0.36616056049520934, 0.3555690901445696, 0.2797625914612995,
        0.23791161208013414, 0.2108694196672318, 0.20931572458473804,
        0.1893809643488431, 0.23658665413180105, 0.2824856280598867, 0.6515929889028145,0], c='r')
    ax1.plot(therange, [0.43917592966100666, 0.4815459809930101, 0.4656227212095525,
        0.515956212946203, 0.3860204700217843, 0.3917625554851447, 0.15677326845462922,
        0.11727802453393973, 0.053654167343434186, 0.10872557391807938,
        0.07465173000359007, 0.25886351399191204, 0.5161494682611578,0], c='g')
    ax1.plot(therange, [0.332212711688271, 0.38933728712367227, 0.4599213420580314,
        0.4922151651020238, 0.43118954364611184, 0.29587252622353705,
        0.26435973989568134, 0.285393232705394, 0.19140072549288095, 0.2656327649081968,
        0.24253216164152192, 0.5033699684665649,0,0], c='b')
    
    ax1.set_xlabel(r'$t/t_{\rm{ff}}$')
    ax1.set_ylabel(r'$\alpha_O,\alpha_B,\alpha_C$')
    fig.savefig('simTwo')
    print(" saved")
    plt.close(fig)
    ...
    fig, ax1=plt.subplots(1,1) 
    therange = np.arange(0,1,0.075)
    ax1.plot(therange, [0.26810199541557084, 0.37336658985265947, 0.31601560199711987,
        0.22287182134015635, 0.15266042046554246, 0.131148850072013, 0.168385515423964,
        0.20693649393568236, 0.15115183423431697, 0.26975332848558276,
        0.25714117422611543, 0.45886348717225284,0,0], c='r')
    ax1.plot(therange, [0.18663925096617345, 0.1981549542653741, 0.31875096326208424,
        0.3902308150406913, 0.3694736780065844, 0.3082023868857484, 0.2332606476208539,
        0.2046894724716348, 0.18211302074748462, 0.2017198100992626,
        0.29737559863443014, 0.3117256983237606, 0.343062364656944,0], c='g')
    ax1.plot(therange, [0.27640071684166767, 0.29120839706784923, 0.3647661213632449,
        0.40840650197511386, 0.3925690437379898, 0.3687179310518396, 0.3397138348837921,
        0.2788021457950369, 0.2559752034157832, 0.23210332177463747,
        0.28965983971373077, 0.6045590979416262,0,0], c='b')

    ax1.set_xlabel(r'$t/t_{\rm{ff}}$')
    ax1.set_ylabel(r'$\alpha_O,\alpha_B,\alpha_C$')
    fig.savefig('simThree')
    print(" saved")
    plt.close(fig)
