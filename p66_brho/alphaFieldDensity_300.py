...
class BRho_tool():
    def __init__(self,this_looper): 
        ...
        # need to add 'self' to the following: 
    ...
    def boxes(self,num,name,tff_lab): 
        ...
        if nt == 0:
            ...
            ax1.plot([1,14],[0.667,0.667],'m--',c=[0.5]*4)
            ax1.plot([1,14],[0.5,0.5],'--',c=[0.5]*4)
            ax1.plot([1,14],[0.4,0.4],'--',c=[0.5]*4)
        ...
        fig.savefig('BearsBoxplot_test%s'%name)
        print("BOX SAVED")
        plt.close(fig)
...
if 'BRho_tool1' not in dir() or clobber:
    #BRho_tool1=BRho_tool(TLM.u301)
    BRho_tool1=TLM.loops['u301']
    simname1 = 'u301'

if 'BRho_tool2' not in dir() or clobber:
    #BRho_tool2=BRho_tool(TLM.u302)
    BRho_tool2=TLM.loops['u302']
    simname2 = 'u302'
  
if 'BRho_tool3' not in dir() or clobber:
    #BRho_tool3=BRho_tool(TLM.u303)
    BRho_tool3=TLM.loops['u303']
    simname3 = 'u303'
...
if 1:
    for nt,tool in enumerate([BRho_tool1,BRho_tool2,BRho_tool3]):
        ...
        if nt == 0:
            ...
            #core_list = [70,275,297,165]
        if nt == 1:
            tff_p = thtr.times[:-1]/t_ff 
            frames = [1,3,5,7,9,11] 
            #core_list = [114,190,192]
        if nt == 2:
            tff_p = thtr.times[:-1]/t_ff 
            frames = [1,3,5,6,8,10] 
            #core_list = [128,165,89]
        ...
        if which_plot == 'frame_scatters': 
            ...
            tool.run(simnames[nt],thtr,which_time,which_plot,
                 fig,ax1,ax2= None,ax3= None,ax4= None,ax5= None,ax7= None,ax8= None,
                 lplots= None,core_list=core_list,simframes= None) 
